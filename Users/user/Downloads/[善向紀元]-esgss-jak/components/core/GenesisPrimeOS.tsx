...
const HolographicHUD: React.FC<HolographicHUDProps> = ({ language, isExpanded, onToggleExpand }) => {
  const { systemVitals, currentPersona } = useContext(UniversalAgentContext);
  ...
  return (
    <motion.div
      className={`fixed top-4 right-4 z-50 bg-black/90 backdrop-blur-md border border-cyan-500/30 rounded-lg overflow-hidden ${
        isExpanded ? 'w-96 h-64' : 'w-64 h-32'
      }`}
            ...
            <div key={quest.id} className="bg-gray-800/50 rounded p-2">
              <div className="flex justify-between items-start mb-1">
                ...
                <span className={`text-xs px-1 py-0.5 rounded ${
                  quest.type === 'main' ? 'bg-blue-500/20 text-blue-400' :
