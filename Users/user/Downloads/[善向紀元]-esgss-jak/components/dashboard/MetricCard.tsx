...
export const MetricCard: React.FC<MetricCardProps> = ({
  ...
  const formattedValue = useMemo(() => {
    ...
    const options = FORMAT_OPTIONS[data.format || 'number'];
        ...
        <div className="flex items-center gap-2">
          ...
          {showTrend && trendIcon && (
            <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
              data.trend === 'up' ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' :
      ...
      {showTarget && data.metadata?.target && (
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className={`font-medium ${
            Number(data.value) >= Number(data.metadata.target) ?
