...
async function fetchWithRetry(url: string, options: any, retries: number, timeout: number, onRetry: (attempt: number, err: any, delay: number) => void): Promise<any> {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                if (!isRetryable) {
                    ...
                    const error = new Error(`HTTP ${res.status} ${res.statusText}${errorText ? `: ${errorText.substring(0, 100)}` : ''}`);
...
export async function runMcpAction(
  ...
  try {
      ...
      if (action === 'api_call') {
          const method = (inputParams.method || 'GET').toUpperCase();
          ...
          try {
              const result = await fetchWithRetry(url, { 
                  method, 
                  headers: inputParams.headers || { 'Content-Type': 'application/json' }, 
                  body: inputParams.body ? JSON.stringify(inputParams.body) : undefined 
              }, 3, 15000, (attempt, err, delay) => {
                  const msg = err.name === 'AbortError' ? 'Timeout' : err.message;
                  onLog?.(isZh ? `[重試] 第 ${attempt} 次 (${msg}) - 等待 ${delay}ms` : `[RETRY] Attempt ${attempt} (${msg}) - Waiting ${delay}ms`, 'warning');
              });
      ...
      const promptMap: Record<string, string> = {
          ...
          perform_collective_benchmarking: `Compare tenant efficiency ${inputParams.currentTenant?.efficiency} against industry peers. Output JSON with roadmap.`,
      ...
      onLog?.(isZh ? `[系統] 執行核心協議：${action}` : `[SYSTEM] Executing protocol: ${action}`, 'info');
      ...
      if (action === 'perform_entropy_transmutation' && (errMsg.includes('429') || errMsg.includes('RESOURCE_EXHAUSTED'))) {
...
export async function performMapQuery(query: string, language: Language): Promise<{text: string, sources?: any[]}> {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    const response = await ai.models.generateContent({
...
export async function performWebSearch(query: string, language: Language): Promise<{text: string, sources?: any[]}> {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: query,
...
export async function generateEsgQuiz(term: string, definition: string, language: string): Promise<any> {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
...
export async function generateNoteSummary(content: string, language: Language, contextStrings: string[]): Promise<string> {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
...
export async function analyzeMedia(mediaUrl: string, prompt: string, mimeType: string): Promise<string> {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: prompt
...
export const JunAiKeyAPI = {
    v1: {
        ...
        cognition: { 
            reason: async (prompt: string, history: any[], thinking: boolean) => {
                const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
                const response = await ai.models.generateContent({
                    model: 'gemini-3-pro-preview',
                    contents: prompt,
                    config: thinking ? { thinkingConfig: { thinkingBudget: 1024 } } : undefined
                });
