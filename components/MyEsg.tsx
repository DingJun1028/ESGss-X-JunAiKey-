...
const BerkeleyHeroBanner: React.FC<{ isZh: boolean, onNavigate: (view: View) => void }> = ({ isZh, onNavigate }) => (
    <div className="relative w-full rounded-2xl overflow-hidden shadow-2xl shadow-amber-900/20 border border-celestial-gold/30 mb-8 group cursor-pointer" onClick={() => onNavigate(View.ACADEMY)}>
        ...
        <div className="relative z-10 p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div className="flex-1">
                <div className="flex flex-wrap gap-2 mb-3">
                    ...
                    <span className="px-2 py-0.5 bg-emerald-500/20 text-emerald-400 text-[10px] font-bold rounded-full uppercase tracking-wider border border-emerald-500/30">TSISDA</span>
                </div>
                ...
                <div className="flex gap-4 mt-4 text-xs text-gray-300">
                    ...
                    <div className="flex items-center gap-1"><Users className="w-3 h-3 text-celestial-purple"/> Silicon Valley Mentors</div>
                </div>
            </div>
            <button className="shrink-0 px-6 py-3 bg-celestial-gold hover:bg-amber-400 text-black font-bold rounded-xl transition-all flex items-center gap-2 group/btn">
...
const WidgetContainer: React.FC<{ 
    ...
    return (
        <div className={`${colSpanClass} glass-panel rounded-2xl border border-white/10 relative group h-full flex flex-col transition-all duration-300 ${isEditing ? 'border-dashed border-celestial-gold/30 bg-celestial-gold/5' : ''}`}>
            {isEditing && (
...
export const MyEsg: React.FC<MyEsgProps> = ({ language, onNavigate }) => {
      ...
      if (size === 'small') {
          return (
              <div className="p-4 flex flex-col items-center justify-center h-full text-center relative group/profile cursor-pointer" onClick={() => onNavigate(View.SETTINGS)}>
                  <div className="w-20 h-20 rounded-full bg-gradient-to-tr from-celestial-purple to-blue-500 p-[2px] mb-3 group-hover/profile:scale-105 transition-transform">
                  ...
                  <h3 className="text-lg font-bold text-white truncate w-full px-2">{userName}</h3>
                  <div className="text-xs text-celestial-gold mt-1 bg-celestial-gold/10 px-2 py-0.5 rounded-full border border-celestial-gold/20">
                      Lv.{level} ({xp} XP)
                  </div>
              </div>
          );
              ...
              {quests.map(quest => (
                  <div key={quest.id} className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5">
                      <div className="flex items-center gap-3">
                          <div className={`p-2 rounded-lg ${quest.status === 'completed' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/10 text-gray-400'}`}>
                              <CheckSquare className="w-4 h-4" />
                          </div>
                          <div>
                              <div className={`text-sm font-medium ${quest.status === 'completed' ? 'text-gray-500 line-through' : 'text-white'}`}>{quest.title}</div>
                              <div className="text-xs text-gray-500">{quest.desc}</div>
                          </div>
                      </div>
                      {quest.status !== 'completed' && (
                          <button 
                              onClick={() => completeQuest(quest.id, quest.xp)}
                              className="px-3 py-1 bg-celestial-purple/20 text-celestial-purple text-xs font-bold rounded-lg border border-celestial-purple/30 hover:bg-celestial-purple/30 transition-all"
                          >
                              Claim
                          </button>
                      )}
      ...
      <div className="p-6 flex flex-col h-full">
          ...
          <form onSubmit={handleAddTodo} className="flex gap-2 mb-4">
              <input 
                  ...
                  className="flex-1 bg-slate-900/50 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-500/50"
              />
              <button type="submit" className="p-2 bg-emerald-500/20 text-emerald-400 rounded-lg border border-emerald-500/30 hover:bg-emerald-500/30 transition-all">
                  <Plus className="w-4 h-4" />
              </button>
          </form>

          <div className="space-y-2 flex-1 overflow-y-auto custom-scrollbar">
              {todos.length === 0 && <div className="text-center text-xs text-gray-500 py-4">{isZh ? '無待辦事項' : 'No tasks'}</div>}
              {todos.map(todo => (
                  <div key={todo.id} className="group flex items-center justify-between p-2 hover:bg-white/5 rounded-lg transition-colors">
                      <div className="flex items-center gap-2">
                          <button onClick={() => toggleTodo(todo.id)} className={`w-4 h-4 rounded border flex items-center justify-center ${todo.done ? 'bg-emerald-500 border-emerald-500' : 'border-gray-500'}`}>
                              {todo.done && <Plus className="w-3 h-3 text-black rotate-45" />}
                          </button>
                          <span className={`text-sm ${todo.done ? 'text-gray-500 line-through' : 'text-gray-300'}`}>{todo.text}</span>
                      </div>
  ...
  const renderYangBoWidget = () => (
      <div className="p-6 flex flex-col h-full bg-gradient-to-br from-celestial-gold/5 to-transparent relative group overflow-hidden" onClick={() => onNavigate(View.YANG_BO)}>
          <div className="absolute top-0 right-0 p-4 opacity-10">
              <Crown className="w-20 h-20 text-celestial-gold" />
          </div>
          ...
          <div className="flex-1 flex flex-col justify-between relative z-10">
              <div>
                  <div className="text-xs text-celestial-gold uppercase font-bold tracking-wider mb-2">Latest Insight</div>
                  <h4 className="text-base font-bold text-white leading-tight mb-2 group-hover:text-celestial-gold transition-colors">
              ...
              <div className="flex gap-2 mt-4">
                  <span className="text-[10px] px-2 py-1 bg-white/5 rounded border border-white/10 text-gray-300">Podcast</span>
                  <span className="text-[10px] px-2 py-1 bg-white/5 rounded border border-white/10 text-gray-300">Strategy</span>
              </div>
          </div>
          <div className="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
              <ArrowRight className="w-5 h-5 text-celestial-gold" />
          </div>
      </div>
  );
      ...
      <div className="p-6 flex flex-col h-full">
          ...
          <div className="space-y-3 flex-1 overflow-y-auto custom-scrollbar">
              {[
                  ...
                  <div key={i} className="flex items-center gap-3 p-3 bg-white/5 rounded-xl border border-white/5 hover:bg-white/10 transition-colors cursor-pointer" onClick={() => onNavigate(View.ACADEMY)}>
                      <div className="flex flex-col items-center justify-center p-2 bg-celestial-purple/10 rounded-lg min-w-[50px] border border-celestial-purple/20">
                          <span className="text-[10px] font-bold text-celestial-purple uppercase">{evt.date.split(' ')[0]}</span>
                          <span className="text-sm font-bold text-white">{evt.date.split(' ')[1]}</span>
                      </div>
                      <div>
                          <div className="text-xs text-gray-400 mb-0.5">{evt.type}</div>
  ...
  const renderWidgetContent = (widget: DashboardWidget) => {
      switch(widget.type) {
          ...
          default: return <div className="p-6">Unknown Widget</div>;
      }
  };
    ...
    <div className="space-y-8 animate-fade-in pb-12 relative">
        ...
        <div className="flex justify-end items-center mb-4 gap-4">
            <button 
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${isEditing ? 'bg-celestial-gold text-black shadow-lg' : 'bg-white/5 hover:bg-white/10 text-gray-400'}`}
            >
                <Edit3 className="w-3 h-3" />
                {isEditing ? (isZh ? '完成編輯' : 'Done Editing') : (isZh ? '自訂版面' : 'Customize Layout')}
            </button>
            {isEditing && (
                <button 
                    onClick={() => setShowCatalog(true)}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold bg-celestial-purple text-white shadow-lg transition-all animate-pulse"
                >
                    <Plus className="w-3 h-3" />
                    {isZh ? '新增小工具' : 'Add Widget'}
                </button>
            )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-[300px]">
            ...
            {isEditing && (
                <div 
                    onClick={() => setShowCatalog(true)}
                    className="col-span-1 border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center text-gray-500 hover:text-white hover:border-white/30 cursor-pointer transition-all h-full bg-white/5"
                >
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
                <div className="bg-slate-900 border border-white/10 rounded-3xl w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col shadow-2xl">
                    ...
                    <div className="p-6 overflow-y-auto custom-scrollbar grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <button 
                            onClick={() => { addMyEsgWidget({ type: 'quest_list', title: 'Daily Quests', gridSize: 'medium' }); setShowCatalog(false); }}
                            className="flex items-center gap-4 p-4 rounded-xl border border-white/10 bg-white/5 hover:bg-celestial-gold/10 hover:border-celestial-gold/30 transition-all text-left group"
                        >
                            <div className="p-2.5 rounded-lg bg-celestial-gold/20 text-celestial-gold">
                            ...
                            <div>
                                ...
                                <div className="text-xs text-gray-400">Daily Tasks (Medium)</div>
                            </div>
                            <Plus className="w-5 h-5 text-gray-500 group-hover:text-white ml-auto" />
                        </button>
